...
This repo builds `picoclaw`, runs the PicoClaw gateway automatically inside the container, and keeps OpenSSH available for direct admin access over Railway TCP.
...
2. Expose a Railway TCP service pointing at internal port `2222`
3. Connect with `ssh <SSH_USERNAME>@<host> -p <port>` for admin access
4. Run `picoclaw onboard` over SSH to finish provider setup
...
5. The gateway starts automatically and listens inside the container on `127.0.0.1:18790`

## Accessing the gateway

- Inside the SSH session, the gateway is available at `http://127.0.0.1:18790`
- It is not exposed publicly by this image
- If you want public access later, put a separate proxy in front of it or tunnel it over SSH
...
- from inside SSH, use `http://127.0.0.1:18790`
